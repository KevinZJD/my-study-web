(function() {
    ...
    style.innerHTML = `
        ...
        .type-mcq { background: #e0e7ff; color: #4f46e5; }
        .type-tf { background: #fef08a; color: #ca8a04; }
    ...
    function renderCompletionScreen(total, time) {
        dom.quizBox.innerHTML = `
            <div class="glass-panel" style="text-align:center;padding:40px;">
                ...
                <div style="font-size:20px;margin:20px 0;"><p>得分：<span style="color:#10b981;font-weight:800;">${score}</span> / ${total}</p></div>
    ...
    async function showQuestion(list) {
        if (quizIdx >= list.length) {
            ...
            renderCompletionScreen(list.length, time); return;
        }
        ...
        if (qType === 'MCQ' || qType === 'TF') {
            ...
            document.getElementById('show-ans-btn').onclick = () => {
                ...
                const handleSelfGrade = (isCorrect) => {
                    ...
                    quizIdx++; showQuestion(list); // 打分后丝滑跳入下一题
                };
